...
public class CreateSalesmanRequest {
    ...
    @Schema(description = "Salesman password", example = "securePassword123", required = true)
    ...
    @Schema(description = "Whether the salesman is enabled", example = "true", defaultValue = "true")
    private Boolean enabled = true;

    @NotNull(message = "Opening balance is required")
    @DecimalMin(value = "0.00", message = "Opening balance must be greater than or equal to 0.00")
    @Schema(description = "Opening balance for the salesman", example = "0.00", required = true)
