...
public class SupplierService {
    ...
    private static final String UNKNOWN_CONTACT_NUMBER = "9999999999";
    ...
    public SupplierResponse create(@Valid CreateSupplierRequest request) {
        ...
        // Check for duplicate contact number (skip if it's the unknown contact number)
        if (request.getContactNumber() != null
                && !UNKNOWN_CONTACT_NUMBER.equals(request.getContactNumber())
                && repository.existsByContactNumberAndPumpMaster_Id(request.getContactNumber(), request.getPumpMasterId())) {
            throw new PumpBusinessException("DUPLICATE_CONTACT_NUMBER",
                    "Supplier with contact number '" + request.getContactNumber() + "' already exists for this pump master");
        }
    ...
    public SupplierResponse update(@NotNull UUID id, @Valid UpdateSupplierRequest request) {
        ...
        // Check for duplicate contact number if contact number is being updated (skip if it's the unknown contact number)
        if (request.getContactNumber() != null
                && !UNKNOWN_CONTACT_NUMBER.equals(request.getContactNumber())
                && !request.getContactNumber().equals(existingSupplier.getContactNumber())
                && repository.existsByContactNumberAndPumpMaster_IdAndIdNot(request.getContactNumber(),
                        existingSupplier.getPumpMaster().getId(), id)) {
            throw new PumpBusinessException("DUPLICATE_CONTACT_NUMBER",
                    "Supplier with contact number '" + request.getContactNumber() + "' already exists for this pump master");
        }
