...
@Repository
public interface ManagerRepository extends JpaRepository<User, UUID> {

    @Query("SELECT u FROM User u WHERE u.role.roleName = 'MANAGER' AND u.pumpMaster.id = :pumpMasterId")
    List<User> findByPumpMasterId(@Param("pumpMasterId") UUID pumpMasterId);

    @Query("SELECT u FROM User u WHERE u.role.roleName = 'MANAGER' AND u.pumpMaster.id = :pumpMasterId AND u.username = :username")
    User findByUsernameAndPumpMasterId(@Param("username") String username, @Param("pumpMasterId") UUID pumpMasterId);

    @Query("SELECT COUNT(u) FROM User u WHERE u.role.roleName = 'MANAGER' AND u.pumpMaster.id = :pumpMasterId")
