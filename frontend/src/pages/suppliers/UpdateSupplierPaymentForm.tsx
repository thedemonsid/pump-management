...
const UpdateSupplierPaymentSchema = z.object({
...
type UpdateSupplierPaymentFormData = z.infer<
...
export function UpdateSupplierPaymentForm({
  payment,
  onSuccess,
}: UpdateSupplierPaymentFormProps) {
  const { editPayment, loading } = useSupplierPaymentStore();
  ...
  const form = useForm<UpdateSupplierPaymentFormData>({
    resolver: zodResolver(UpdateSupplierPaymentSchema),
    defaultValues: {
      pumpMasterId: payment.pumpMasterId,
      supplierId: payment.supplierId,
      purchaseId: payment.purchaseId || '',
      fuelPurchaseId: payment.fuelPurchaseId || '',
  ...
  const onSubmit = async (data: UpdateSupplierPaymentFormData) => {
    ...
    try {
      await editPayment(payment.id, {
        ...
        paymentMethod:
          data.paymentMethod as (typeof SupplierPaymentMethod)[keyof typeof SupplierPaymentMethod],
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input placeholder="Enter fuel purchase ID" {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
