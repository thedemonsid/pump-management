...
export function UpdatePaymentForm({
  ...
  const form = useForm<UpdateEmployeeSalaryPayment>({
    defaultValues: {
      calculatedSalaryId: payment.calculatedSalaryId || null,
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const [salaries, payments] = await Promise.all([
          CalculatedSalaryService.getByUserId(payment.userId),
          EmployeeSalaryPaymentService.getByUserId(payment.userId),
        ]);

        // Get list of already linked salary IDs (excluding current payment's link)
        const linkedSalaryIds = new Set(
          payments
            .filter((p) => p.calculatedSalaryId && p.id !== payment.id)
            .map((p) => p.calculatedSalaryId!)
        );

        // Filter out already linked salaries (but keep the one currently linked to this payment)
        const availableSalaries = salaries.filter(
          (salary) =>
            !linkedSalaryIds.has(salary.id!) ||
            salary.id === payment.calculatedSalaryId
        );
    ...
    fetchData();
  }, [payment.userId, payment.id, payment.calculatedSalaryId]);
  ...
  const onSubmit = async (data: UpdateEmployeeSalaryPayment) => {
    try {
      ...
      await EmployeeSalaryPaymentService.update(payment.id!, paymentData);
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="sm:max-w-[600px] overflow-y-auto">
        <SheetHeader>
          <SheetTitle>Update Salary Payment</SheetTitle>
          <SheetDescription>
            Update the payment details for this salary payment.
          </SheetDescription>
        </SheetHeader>
